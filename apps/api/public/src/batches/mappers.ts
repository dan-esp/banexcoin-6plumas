...
export function mapBatch(batch: Batch, result?: CashbackResult | null) {
  ...
  const approvedAt = batch.approval?.approvedAt ?? null
  const approvedBy = batch.approval?.approvedBy ?? null
  const exportedAt = batch.exportMetadata?.exportedAt ?? null
  const exportReady = Boolean(batch.exportMetadata)
  ...
  return {
    ...
    export: {
      ready: exportReady || (approved && blockedRows === 0),
